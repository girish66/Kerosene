...
namespace MB.KeroseneORM.Direct
{
	...
	public class KLinkDirect : KLink, IKLink
	{
		...
		internal object TransformParameterValue( object value )
		{
			...
			if( ExtendedInfo.ContainsKey( key ) ) {
				object transformer = ExtendedInfo[key];
		...
		public void AddParameterTransformer<T>( Func<T, object> transformer, bool replace = false )
		{
			if( transformer == null )
				throw new ArgumentNullException( "transformer", "Parameter Transformer delegate cannot be null." );

			Type type = typeof( T );
			string key = _GetParameterTransformerKey( type );

			if( ExtendedInfo.ContainsKey( key ) && !replace )
				throw new InvalidOperationException( "Parameter Transformer delegate already registered for type: " + type.FullName );

			ExtendedInfo[key] = transformer;
		}
		...
		public void RemoveParameterTransformer<T>()
		{
			Type type = typeof( T );
			string key = _GetParameterTransformerKey( type );
		...
		/// </summary>
		public void ClearParameterTransformers()
		{
			if( ExtendedInfo.Count != 0 ) {
				string[] array = ExtendedInfo.Keys.ToArray();
